...
import numpy as np
import argparse
from Optimizer.AMDA import AMDA
...
def build_dataset():
    ...
    testset = torchvision.datasets.CIFAR10(root='./data_CIFAR10', train=False, download=True,
...
def select_optimizer(optimizer_name, net, train_loader):
    ...
    elif optimizer_name == 'AMDA':
        return AMDA(net.parameters(), lr=1e-3, weight_decay=5e-4)
    else:
...
def main():
    ...
    parser.add_argument('--optimizer', default='AMDA', type=str, help='optimizer: SGD, Adam, AdamW, AMDA')
